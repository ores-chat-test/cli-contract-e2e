...
test('rejects a genuinely missing provider lane', () => { const path = root(); providerTree(path); rmSync(join(path, 'neon/admin/migrations'), { recursive: true, force: true }); const findings = auditInfra(path); assert.ok(findings.some(({ code, target }) => code === 'required-path-missing' && target === 'neon/admin/migrations')); });
...
test('rejects embedded PostgreSQL URLs in migration SQL', () => { const path = root(); providerTree(path); writeFileSync(join(path, 'supabase/auth/migrations/202609110001_policy.sql'), '-- postgres://example.invalid/db\nselect 1;\n'); assert.ok(codes(path).includes('infra-migration-sql-database-url')); });
